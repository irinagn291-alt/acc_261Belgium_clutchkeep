# Clutchkeep — Build Specification
...
**One-line positioning:** Tap a hen to log an egg and keep the yard book.

| Field | Value |
| --- | --- |
| Product name | Clutchkeep |
| Bundle identifier | `com.clutchkeep.roost` |
| Domain | https://clutchkeep-roost.pro |
| Contact URL | https://clutchkeep-roost.pro/contact-us |
| Deployment target | iOS 17.0 |
| Swift version | 6.2, strict concurrency `complete` |
| Devices | iPhone and iPad, portrait |
| Interface style | Light |
| Asset prefix | `ckp_` |
| User-Agent | `Clutchkeep/1.0 (iOS; +https://clutchkeep-roost.pro)` |
...
8. **The app must build clean** with
   `xcodegen generate && xcodebuild -scheme Clutchkeep -destination 'generic/platform=iOS' build`.
...
A backyard keeper logs each egg onto a hen on the roost so the yard's yield, feed cost and health score update.

### 2.1 User flow

1. Tap a hen on the roost to log one egg
2. Add a hen on the roost if the yard is empty
3. Open the Mash page and log today's feed kilograms and cost
4. Open the Dose page and note a health event on a hen
5. Open Analytics to read cost per egg, FCR and healthScore
6. Cull a hen when she leaves the flock
7. Open Settings to export the yard book or reset

### 2.2 Essential behaviour

- Roost of named hens; a nest tap writes one Credit
- Pullet becomes Layer on first Credit via a LayMark
- Mash entries with kilograms and cost; costPerUnit and FCR use Credit qty
- Dose notes on a hen; healthScore from mortality and notes
- Distinct empty states for no hens versus no credits
- Analytics by eggs per Layer and cost versus yield, not a typed basket total
- Local-only yard book with CSV export and no marketplace

---

## 3. Uniqueness assignment for Clutchkeep

| Axis | Assigned value |
| --- | --- |
| Architecture | **Roost-credit encoding (a tap writes Credit; first Credit writes LayMark and flips Pullet to Layer; Cull drops the hen from the divisor)** |
| UI approach | **SwiftUI pure · take flockledger** |
| Naming convention | **Poultry / roost lexicon** |
| File organization | **By roost role (Hen, Credit, Mash, Dose, LayMark)** |
| Dependency strategy | **None** |
| Design direction | **Desert dune editorial** |
| Typography | **SF Pro** |
| Navigation pattern | **Roost-tab chrome (the Flock tab holds the roost; mash and doses fuse on the pager; Analytics and Settings are sibling tabs)** |
| AI art style | **3D glass render glassmorphism · take flockledger** |
| Functional twist | **Pullet-then-layer (a tap writes a Credit; first Credit writes a LayMark and flips Pullet to Layer; qty is Credits)** |
...
**Core** — A backyard keeper logs each egg onto a hen on the roost so the yard's yield, feed cost and health score update.

**Audience** — Small keepers who need a yard book, not farm ERP.
...
**Twist** — Pullet-then-layer. Home is the roost. A tap writes one Credit on that hen. The first Credit writes a LayMark and flips Pullet to Layer. Cull removes her from tomorrow's divisor. qty is the Credit count. costPerUnit is mash cost over qty. FCR is mash kilograms over qty. There is no flock-size field and no basket-count field. Home verb: credit-the-hen, not type-a-basket. Analytics count LayMarks and Credits.

**Why this is not a repeat** — HenTrack logs a flock-size integer and a daily egg count on a form, then divides for eggs/hen and a 7-day predict. This product has neither field: yield is a fold over Credits on named hens, and a Pullet cannot become a Layer without a LayMark. It is not Waywiser, because a tap does not move two registers; it writes an egg and maybe a LayMark. It is not a habit tracker with a chicken icon and not farm ERP. Mash and health stay inline on the Flock pager. The leftover scanner and cgi search endpoint stay unused so this does not become a food barcode app.
...
The yard book is a roost-credit encoding: named Hens sit the roost, a nest tap writes one Credit on that hen, and qty is the Credit count rather than a flock-size integer or a basket field. The first Credit on a Pullet writes a LayMark and flips her role to Layer; later Credits never write another LayMark. Cull removes the hen from tomorrow's Layer divisor so eggs-per-Layer folds only over remaining Layers. Mash entries hold kilograms and cost; costPerUnit is the sum of mash cost over qty and FCR is the sum of feedKg over qty, both empty when qty is 0. Dose notes hang on a hen; healthScore is clamp of 100 minus mortality times 140 minus min of 20 and notes. One RoostStore owns the fold; views call creditHen, logMash, noteDose, and cullHen; unit tests name costPerUnit, feedKg, healthScore, and the LayMark flip.
...
100% SwiftUI with the flockledger take-token: new roost composition, new types, new motion — steal HenTrack eggs-per-layer density, 7-day predict, flock-versus-credits empty states, and CSV only; do not copy HenTrack forms, flock-size fields, type names, file trees, WebView, OneSignal, or AppsFlyer. No UIViewRepresentable, no camera preview, no Safari sheet. Home is the mechanic: Flock is a roost of named hen tiles filling remaining height and the iPad width; a nest tap writes a Credit. KPI tiles use Springboard zoom into Production, Mash, and Dose; Mash and Dose also sit as fused pager pages on Flock. Confine custom drawing to that one roost hero (Shape, Path, hairline glass tiles); Analytics and Settings are stock TabView, NavigationStack, List, Form. Primary actions use borderedProminent with a 1pt hairline-plus-fill surface, min 44pt, contentShape on the fill. Empty flock, empty credits, and onboarding are full pages (frame maxHeight infinity) with the CTA at the bottom full width. Background fills the safe area; the tab bar sits on the home indicator; lists use contentMargins.bottom. Pullet versus Layer is a LayMark bead plus VoiceOver, never colour alone. One haptic on a successful credit, mash, dose, or cull; none on tab changes.

### 3.3 Naming contract

Convention: Poultry / roost lexicon.

Examples to follow: `Hen`, `Credit`, `creditHen(_:)`, `LayMark`

### 3.4 Dependency contract

Clutchkeep ships a single app target with no packages key in project.yml. Foundation and SwiftUI only. Do not add SPM, CocoaPods, a bundled face, a URLSession catalog, or an AVFoundation capture session. The leftover scanner axis and cgi search pl endpoint stay dark so this never becomes a food barcode app. Yard-book CSV is a local FileManager write. No marketplace.

### 3.5 Navigation contract

Roost-tab chrome: TabView with three sibling tabs Flock, Analytics, and Settings, each owning a NavigationStack. Flock holds the roost and never leaves it for a record stack; a hen tap credits in place; Mash and Dose fuse on the Flock pager and also open when a KPI tile zooms. Analytics and Settings are sibling tabs, not sheets that replace the roost. Contact URL https://clutchkeep-roost.pro/contact-us lives on Settings. No Nest, Scan, Search, or Game tab. After onboarding, read ProcessInfo.processInfo.arguments once: -ReviewScreen today opens Flock, log opens Analytics, goals opens Settings — three different screens. Skip onboarding on Simulator after the ckp.demo.v1 seed so the hook can fire.

### 3.6 Screen composition contract

Springboard zoom tiles. Physical screens: Flock, Analytics, Settings. Flock is home: a roost of named hen tiles plus KPI tiles that zoom into Production, Mash, and Dose then zoom back; nest tap writes one Credit; Mash kilograms and cost and Dose notes fuse on the Flock pager; ReviewScreen today. Empty Flock with no hens is a full page with generated cutout art, headline No flock yet., one line Add the birds first., and a full-width bottom Add a hen CTA. Hens with no Credits use a distinct full-page empty nest (not the empty roost), headline No credits yet., one line Tap a hen to log an egg., CTA on the roost. Analytics is a sibling tab of eggs per Layer, cost versus yield, costPerUnit, FCR, healthScore, LayMark count, Credit count, and a 7-day predict folded from Credits, not a typed basket total (ReviewScreen log), with its own full-page empty. Settings is a Form for CSV export, contact URL at https://clutchkeep-roost.pro/contact-us, re-run onboarding, and resetAllData (ReviewScreen goals). Onboarding is a one-shot cover of three pages with Continue or Next at the bottom full width. No Today, Scan, Search, or Goals screens. No Nest mini-game. ReviewScreen today log and goals must open three different screens.
...
Scheme: **By roost role (Hen, Credit, Mash, Dose, LayMark)**
...
Direction: **Desert dune editorial**
...
| `ink` | `#183936` | Primary text and icons |
| `accent` | `#25C1B4` | Primary action, key figure, progress fill |
| `muted` | `#547875` | Secondary text, dividers, disabled |
...
SF Pro via Font.system for Clutchkeep. Display is Credit qty and healthScore on the roost KPI strip. Body is hen names on tiles. Caption is Pullet and Layer roles. Tabular figures for qty, mash kilograms, mash cost, costPerUnit, FCR, and healthScore through NumberFormatter. At most six steps behind one accessor. No Font.custom and no fixedSize. Never below 12pt. Dynamic Type; hen names truncate, numbers win. Day edges use Calendar.current.startOfDay then fold to Int YYYYMMDD.
...
Corner radius: **14pt** for cards, sheets and primary surfaces; **10pt** for chips, badges and small controls. Reach both through one accessor. Never a bare literal number, and never zero — a hard edge is not this app's design direction.
...
This app's `ui` axis is **SwiftUI pure · take flockledger**.
...
If **SwiftUI pure · take flockledger** is already fully native (no custom drawing layer), this section is satisfied automatically — there is nothing to confine.
...
One Codable YardBook (schemaVersion from 1, Hens with Pullet or Layer role, Credits, Mash, Doses, LayMarks, culls, and day keys as Int YYYYMMDD) encoded to JSON Data in UserDefaults under ckp.yard.v1. qty, costPerUnit, FCR, eggs-per-Layer, and healthScore are derived at display and never stored. In-memory RoostStore is the source of truth; UserDefaults is the projection. Debounce writes; flush when scenePhase becomes inactive or background; encode after every credit, mash, dose, and cull. Decoding failure falls back to an empty roost, never a crash. resetAllData() is reachable from Settings. Simulator seed only once behind ckp.demo.v1 writes several named hens with a mix of Pullet and Layer, several Credits, Mash lines, and Dose notes, leaves credit-the-hen enabled, marks onboarding complete, and never seeds an empty roost as the first frame. Never seed on a device. Views never touch UserDefaults. CSV export reads the same document.
...
Set `User-Agent: Clutchkeep/1.0 (iOS; +https://clutchkeep-roost.pro)` on every request. Never reuse another app's string.
...
## 12. Functional twist: Pullet-then-layer (a tap writes a Credit; first Credit writes a LayMark and flips Pullet to Layer; qty is Credits)

Home is the roost of named hens, not a form with flock size and a typed basket. A tap writes one Credit on that hen; the first Credit writes a LayMark and flips Pullet to Layer. qty is Credits; Analytics counts LayMarks and Credits and shows eggs per Layer, cost versus yield, FCR, healthScore, and a 7-day predict folded from Credits. Cull drops her from tomorrow's divisor; there is no flock-size field and no basket-count field. The home verb is credit-the-hen; Mash and Dose stay inline on the Flock pager. Cover creditHen, the LayMark flip, costPerUnit, feedKg, and healthScore with a unit test on the store, not a View.
...
Art style: **3D glass render glassmorphism · take flockledger**
...
given. Every name carries the `ckp_` prefix.
...
| --- | --- | --- | --- | --- |
| 1 | `ckp_AppIcon` | 1024x1024 | **NO** | App Store icon. NO alpha channel, NO transparency, NO text, NO rounded corners, NO drop shadow outside the canvas. |
| 2 | `ckp_Splash` | 1290x2796 | fill | Launch background. The middle third must stay quiet so the wordmark reads on top. |
| 3 | `ckp_Onboarding1` | 1024x1536 | **required cutout** | Onboarding page 1 illustration: what the app is for. |
| 4 | `ckp_Onboarding2` | 1024x1536 | **required cutout** | Onboarding page 2 illustration: the main verb. |
| 5 | `ckp_Onboarding3` | 1024x1536 | **required cutout** | Onboarding page 3 illustration: why they stay. |
| 6 | `ckp_EmptyHome` | 1024x1024 | **required cutout** | Empty state: the home screen has nothing yet. Calm and inviting, never sad. |
| 7 | `ckp_EmptyList` | 1024x1024 | **required cutout** | Empty state: a secondary list has no rows. |
| 8 | `ckp_CardBackdrop` | 1200x800 | fill | Backdrop art for a primary card. Low contrast so text stays readable. |
| 9 | `ckp_ControlFace` | 512x512 | **required cutout** | Custom control artwork used for the primary interactive element. |
| 10 | `ckp_TwistHero` | 1024x1024 | **required cutout** | Hero art for the 'Pullet-then-layer (a tap writes a Credit; first Credit writes a LayMark and flips Pullet to Layer; qty is Credits)' feature screen. |
| 11 | `ckp_SuccessMark` | 512x512 | **required cutout** | Shown briefly when the primary action succeeds. |
| 12 | `ckp_HeaderDecor` | 1200x600 | **required cutout** | Decorative header accent on the main screen. |

### Prompt per asset

**`ckp_AppIcon`** — 1024x1024

```
A single 3D glass hen on a tiny roost rail, glassmorphism, subject centred filling the canvas edge to edge, no text, no letters, no words, no alpha, no transparency, no rounded corners, no drop shadow outside the canvas
```

**`ckp_Splash`** — 1290x2796

```
A tall vertical 3D glass roost rail of nest cups, frosted glassmorphism, quiet uncluttered centre band for a wordmark, dune-cut studio light, no readable text
```

**`ckp_Onboarding1`** — 1024x1536

```
3D glass still life of a backyard roost with named hen nest cups, what the product is in one glance, isolated cutout, no text
...
**`ckp_Onboarding2`** — 1024x1536

```
3D glass mid-gesture: a finger tapping a nest cup on a hen so one glass egg credits, glassmorphism, isolated cutout, no text

HARD CUTOUT: isolated subject on a fully transparent background. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`ckp_Onboarding3`** — 1024x1536

```
3D glass roost after many credits: Layer hens wearing LayMark beads beside a Pullet still unmarked, accumulated yard, isolated cutout, no text

HARD CUTOUT: isolated subject on a fully transparent background. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`ckp_EmptyHome`** — 1024x1024

```
An empty 3D glass roost rail with no hens seated, waiting to be filled, calm and inviting, never sad, isolated cutout, no text

HARD CUTOUT: isolated subject on a fully transparent background. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`ckp_EmptyList`** — 1024x1024

```
An empty 3D glass nest cup with no Credits seated, hens exist but the nest is unused, calm, isolated cutout, no text

HARD CUTOUT: isolated subject on a fully transparent background. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`ckp_CardBackdrop`** — 1200x800

```
Abstract low-contrast 3D frosted glass roost slats and dune-cut bloom, quiet enough for text on top, filling the canvas, no letters
```

**`ckp_ControlFace`** — 512x512

```
The face of a nest tap as a single physical control: a glass nest cup, 3D glass cutout, isolated, no text

HARD CUTOUT: isolated subject on a fully transparent background. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`ckp_TwistHero`** — 1024x1024

```
3D glass emblem of pullet-then-layer: an unmarked pullet nest beside a Layer nest wearing one LayMark bead, glassmorphism cutout, no text

HARD CUTOUT: isolated subject on a fully transparent background. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`ckp_SuccessMark`** — 512x512

```
A small glass egg seated in a nest cup after a credit, 3D glass cutout, confirmation not fireworks, no letters

HARD CUTOUT: isolated subject on a fully transparent background. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`ckp_HeaderDecor`** — 1200x600

```
A wide low 3D glass band of roost rail and nest cups, frosted glassmorphism, low contrast, no readable text
...
Add a unit test target `ClutchkeepTests` covering at minimum:
...
- [ ] `xcodebuild -scheme Clutchkeep -destination 'generic/platform=iOS' build` succeeds.
...
**Uniqueness**
- [ ] Architecture matches **Roost-credit encoding (a tap writes Credit; first Credit writes LayMark and flips Pullet to Layer; Cull drops the hen from the divisor)** with no leakage across layers.
- [ ] UI approach matches **SwiftUI pure · take flockledger**.
- [ ] Custom rendering, if any, is confined to one hero surface (section 7.5).
- [ ] Navigation matches **Roost-tab chrome (the Flock tab holds the roost; mash and doses fuse on the pager; Analytics and Settings are sibling tabs)**.
